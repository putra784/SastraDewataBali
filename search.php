...
$searchResults = [];
if (!empty($q)) {
    $searchResults = searchAllPosts($q);

    if ($searchResults instanceof mysqli_result) {
        $searchResults = $searchResults->fetch_all(MYSQLI_ASSOC);
    }
}
?>
...
<body>
    ...
    <!-- Search -->
    <section class="mx-14 min-h-screen">
        <form method="GET" action="search.php" class="w-full max-w-md flex gap-2 items-center justify-center my-4 mx-auto">
            <input
                type="text"
                name="q"
                placeholder="Search..."
                class="bg-gray-100 w-full px-4 py-2 rounded-lg border border-gray-300 
                focus:outline-none focus:ring-1 focus:ring-yellow-900 
                focus:shadow-lg transition-all duration-300 hover:bg-white focus:bg-white"
                value="<?= isset($_GET['q']) ? htmlspecialchars($_GET['q']) : '' ?>"
                required />

            <button type="submit">
                <img
                    class="bg-gray-100 rounded-lg border border-gray-300 p-2 cursor-pointer 
                  hover:bg-white transition-all duration-200 ease-in"
        ...
        <h2 class="text-xl font-semibold mb-6">
            Search results for:
            <span class="text-yellow-900">"<?= htmlspecialchars($q) ?>"</span>
        </h2>
            ...
            <?php if (!empty($searchResults)) : ?>
                <?php foreach ($searchResults as $post) : ?>
                    <a href="content.php?id=<?= htmlspecialchars($post['id']); ?>"
                        ...
                        <div class="w-1/2 h-32 overflow-hidden rounded-xl">
