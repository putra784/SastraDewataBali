...
// ambil posts status draft
$posts = getFilteredPosts('draft', $_SESSION["user_id"], $show, $offset, $language, $category, $search);

// total records for pagination
$totalPosts = getFilteredTotalPosts('draft', $_SESSION["user_id"], $language, $category, $search);
    ...
    <div class="w-[260px] bg-[#4e342e] text-white flex flex-col p-6">
      ...
      <div class="mb-3">
        ...
        <div
          ...
          <a
            href="drafts.php"
            class="px-4 py-2 text-sm hover:bg-gray-300 border-b border-gray-500 bg-gray-300">Drafts</a>
          <a
    ...
    <div class="flex-1 flex flex-col">
      ...
      <div class="w-full p-8">
        ...
        <div class="flex justify-between items-center mb-6">
          <h1 class="text-2xl font-semibold tracking-wide">Drafts</h1>
        ...
        <form method="GET" class="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <select name="category" onchange="this.form.submit()" class="px-3 py-2 border rounded text-sm bg-white">
              ...
              <option value="Tradition" <?= $category == 'Tradition' ? "selected" : "" ?>>Tradisi</option>
            ...
            <tbody>
              ...
              <?php foreach ($posts as $post): ?>
                <tr class="border-b hover:bg-gray-50">
                  ...
                  <td class="flex flex-col gap-2 px-3 py-2 border text-center">
                    <div class="flex justify-between gap-2 items-center">
                      <a
                        href="edit-post.php?id=<?= $post['id'] ?>"
                        class="px-3 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700 w-1/2">
                        Edit
                      </a>

                      <a
                        href="delete-post.php?id=<?= $post['id'] ?>"
                        class="px-3 py-1 bg-red-600 text-white rounded text-xs hover:bg-red-700 w-1/2"
                        onclick="return confirm('Are you sure?')">
                        Delete
                      </a>
                    </div>
                    <a href="move-to-pending.php?id=<?= $post['id'] ?>"
                      class="px-3 py-1 bg-yellow-700 text-white rounded text-xs hover:bg-yellow-800"
                      onclick="return confirm('Move this post to Pending?')">
                      Move to Pending
                    </a>
                  </td>
                </tr>
